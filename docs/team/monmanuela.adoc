...
FoodZoom is a condition delivery management desktop application for condition businesses to streamline their
delivery process. FoodZoom is responsive and quick as users interact with the application by typing the
commands (using Command Line Interface). FoodZoom allows managers to plan request delivery routes and
...
* *Major enhancement*: added the ability to assign requests to a healthworker.

** What it does: It allows the manager to assign multiple requests to a healthworker. Subsequently, the
request status changes from `PENDING` to `ONGOING`, which means that it can no longer be edited or
reassigned to another healthworker.

** Justification: This feature streamlines the condition delivery process as the manager can assign requests
to be delivered by their deliverymen conveniently.

** Highlights: The implementation was challenging as it required two-way association between the request and the healthworker.

* *Minor enhancements*:

** Improved checking for the date of the request to prevent invalid dates.
