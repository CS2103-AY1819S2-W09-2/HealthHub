...
specifically targeted towards condition businesses who need an efficient way to manage their delivery services.
...
- Users being able to track condition requests (visualise on a map) they have from customers
- Managing deliverymen and assigning requests to them
- Analysis of requests to display relevant statistics (request history, trending condition etc.) to users
...
requests at a region. The dashboard shows statistics like the most popular condition item and the request history

** Justification: Users of this application can track where their pending requests are in one glance. Orders in the
same region can now be dispatched to the same healthworker. Users can also use the generated statistics to improve their
...
the status of their requests, available deliverymen and key insights for their data all in one screen.
...
** Added essential fields such as address and date to request model
