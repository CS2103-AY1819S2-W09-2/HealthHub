...
Each time there is an update ot the requests, run through the table and see which food item has the largest count value.
...
Adding a new request requires the customer's `name`, customer's `phone` number, customer's `address`, `food` and the `datetime`. +
...
|`* *` |manager |find requests by more files (name, food item etc.) | have more options to locate specific requests
...
... Test case: `/request add n/John Lee p/98765432 a/311, Clementi Ave 2, #02-25, 612344 dt/32-10-2018 00:00:00` +
    Expected: No request is added. Error details shown in the status message (no food). Status bar remains the same.
...
. Finding request by address, name, food
.. Prerequisites: Having at least 1 request.
...
... Test case: Valid arguments +
    ...
    Expected: Food of the request with the `<order_index>` will be overwritten with the food `Roti Canai`
