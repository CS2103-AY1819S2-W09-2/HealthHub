...
This project follows https://github.com/oss-generic/process/blob/master/docs/CodingStandards.adoc[oss-generic coding standards]. IntelliJ's default style is mostly compliant with ours but it uses a different import request from ours. To rectify,

. Go to `File` > `Settings...` (Windows/Linux), or `IntelliJ IDEA` > `Preferences...` (macOS)
. Select `Editor` > `Code Style` > `Java`
. Click on the `Imports` tab to set the request

* For `Class count to use import with '\*'` and `Names count to use static import with '*'`: Set to `999` to prevent IntelliJ from contracting the import statements
* For `Import Layout`: The request is `import static all other imports`, `import java.\*`, `import javax.*`, `import org.\*`, `import com.*`, `import all other imports`. Add a `<blank line>` between each `import`
...
The _Sequence Diagram_ below shows how the components interact for the scenario where the user issues the command `/request delete 1`.

.Component interactions for `/request delete 1` command (part 1)
...
.Component interactions for `/request delete 1` command (part 2)
...
.  `OrderBookParser` further creates either a `OrderCommandParser` or `DeliverymanCommandParser` to parse request or healthworker command respectively. Other commands that is neither request or healthworker skip this creation of the extra Parser.
.  This results in a `Command` object which is executed by the `LogicManager`.
.  The command execution can affect the `Model` (e.g. adding an request) and/or raise events.
.  The result of the command execution is encapsulated as a `CommandResult` object which is passed back to the `Ui`.

Given below is the Sequence Diagram for interactions within the `Logic` component for the `execute("/request delete 1")` API call.

.Interactions Inside the Logic Component for the `/request delete 1` Command
...
.Interactions inside the Storage component for whenever `ModelManager.saveFoodZoom(orderBook, healthworkerList)` is called.
...
`/request add`, `/request edit`, `/request list`, ... .Basically all the `/request` commands and `/healthworker` commands. The
...
** Cons: User is able to type authenticated commands like `/request add` and receive a parse exception which is the
incorrect behavior.
...
..   Map showing location of pending requests
...
The dashboard dynamically updates itself when the request list has been changed. For example,
if `/request find n/john` is executed, the dashboard will also show requests that are specific
to john.
...
```
this.requestList.addListener((ListChangeListener.Change<? extends Order> change) -> {
    while (change.next()) {
        if (change.wasUpdated()) {
            // handle request list being updated
            ....
        } else {
            // handle removal or addition to request list
            ....
        }
    }
});
...
The Map shows the locations of the pending requests.
...
.. The size of the placeholder is indicative of the number of requests in a specific area.

In request to work offline, the map makes use of the postal code that is given
with the request address to give a *reasonable* approximation of its location in Singapore.
...
Based on this information in request to create an offline map, the components/data structures
used are as follows:
...
has no knowledge of changes to the list of requests. Instead, it exposes 2 methods, `initialise`
and `clear` that is used by the higher-request component to update the placeholders.
// end::dashboard[]
...
a Hash Table of how many requests fall under a specific postal code region. An example data that it
...
by the `count` of requests that fall within a specific region.
...
of requests without relying on any internet connection.
...
.. The total number of requests displayed
.. The percentage of requests that are still pending
.. The most common condition item bought
.. The Order History for the last 7 days in which there was at least one request
...
| Total Orders                | Variable       | Count the requests in the list
| Pending Order Percentage    | Variable       | Count requests with status "PENDING" and divide with total requests
| Trending Food Item          | HashTable      | Use a lookup table to track the count of each condition item in menu.
Each time there is an update ot the requests, run through the table and see which condition item has the largest count value.
...
compared with each other. This allows the 7 most recent dates with at least one request to be retrieved quickly. The
tree is updated accordingly when requests are added or removed.
|=======================================================================
// end::dashboard2[]
...
manager will take in the list of requests and return the required statistics needed.

** Pros:
- This solution is more scalable. By having a separate class dedicated to managing the
statistics of requests, it is easier to add in much more
...
==== Current Implementation
The `add` request command allows the `Manager` to create a new request and add it to the list of requests.
Adding a new request requires the customer's `name`, customer's `phone` number, customer's `address`, `condition` and the `datetime`. +
The format of the `add` command is `/request add n/NAME p/PHONE a/ADDRESS c/CONDITION dt/DATETIME`.
...
Figure 3.3.1.1 Sequence Diagram for request `find` command

From the diagram above:

.  `LogicManager`'s execute is called when the `Manager` keys in `/request add` and it calls upon `parseCommand` of `OrderBookParser` to parse the command
.  `OrderBookParser` will initialize `OrderCommandParser` and invoke the method `parse` to further parse `request` command
...
** Pros: The comparison is not as rigid. You can create requests with similar names/address as long as it passes a less stringent criteria.
...
** Cons: It is a strict check and as a result you will not be allowed to create requests with same address/same name etc.
...
==== Current Implementation
The request `find` command allows searching of requests in FoodZoom. It allows searching for requests with any request fields.
...
Figure 3.4.1.1 Sequence Diagram for request `find` command

From the sequence diagram:

.  When `LogicManager` receive the `execute` command, it calls the `parseCommand` method in `OrderBookParser`.
.  `OrderBookParser` will receive `/request` as the command and instantiate `OrderCommandParser` to further parse the command.
...
The differentiation is handled by `OrderPredicateUtil` which checks for the fields provided. It throws `ParseException` if a invalid request field is specified or when any field is given without any value.
...
* **Alternative 1 (current choice):** All different request field having its own test method.
...
=== Add healthworker feature

==== Current Implementation

The `add` healthworker command allows the Manager to add healthworker into FoodZoom. It ensures that the newly-added healthworker
is not a duplicate of an existing healthworker.

The following diagram shows the sequence flow from the `LogicManager` to the `ModelManager` when a user enters a `/healthworker add`
command:

image::DeliverymanAddCommandSequenceDiagram.png[width="650"]
Figure 3.5.1.1 Sequence Diagram for healthworker `add` command

From Figure 3.5.1.1:

1. When `LogicManager` receives the `execute` command from the App, it calls the `parseCommand` method in `OrderBookParser`.

2. `OrderBookParser` will receive `/healthworker` as the command and instantiate `DeliverymanCommandParser` to parse the rest of the command.
...
5. A new healthworker would be added and a successful `CommandResult` would be returned and displayed.

FoodZoom's healthworker is only stored with names currently, and can be search and listed
only with names.

These names are checked against validations with the `Name` model, and this is a compulsory
field for a healthworker to be added. If the provided command either does not have a name, or is an invalid `Name`,
...
** Pros: Allows for greater flexibility in how deliverymen interact with other resources like request/routes - better separation of concerns.
...
This feature is deprecated as there was a design change regarding the relationship between delivery man and request.
Previously, route was the middleman between delivery men and requests. Now, delivery men and requests are directly related.
...
It allows creating routes based on the request ids.
...
=== Assign feature
==== Current Implementation
The `/assign` command allows assigning of multiple requests to a delivery man in FoodZoom.
...
.  Iterate through the `orderIds`, if valid, add the `request` corresponding to that `index` to the set of `Order`.
.  Create a new `Deliveryman` to be assigned from the `healthworker` corresponding to the healthworker index.
.  Iterate through the requests, call `setDeliveryman`.
.  A new route would be added and create a new `CommandResult` to be returned.
...
** Pros: Performing update operations such as assigning requests or marking requests as completed and displaying information on each request and
delivery man are more effective as they are aware of one another.
** Cons: More complicated to implement the assigning of request, and subsequently other features regarding this relationship such as removing of request.
* **Alternative 2:** Only the `Deliveryman` is aware of the `Order`
** Pros: Easier to implement the feature and manage the request and healthworker states.
** Cons: Performing the update operations mentioned above will be ineffective, and it will be more difficult to display information on which delivery man an request is assigned to.
...
*Scenario:* You are in charge of `logic`. During dog-fooding, your team realize that it is troublesome for the user to type the whole command in request to execute a command. Your team devise some strategies to help cut down the amount of typing necessary, and one of the suggestions was to implement aliases for the command words. Your job is to implement such aliases.
...
* Needs to manage a large collection of requests
* Needs to dispatch requests to relevant delivery man
* Wants to view route information and calculate timings
* Wants insights on purchases and request history
* is reasonably comfortable using CLI apps


*Value proposition*: Integrate end-to-end systems and streamline condition delivery management
...
|`* * *` |manager |add a new customer request | can track the current requests I
have

|`* * *` |manager |add or remove a healthworker | can manage the deliverymen available to dispatch requests

|`* * *` |manager |assign requests to available deliverymen | can send out requests and fulfil customer requests

|`* * *` |manager |delete an request | can cancel requests that should not be in the queue

|`* * *` |manager |find an request using phone number | can find specific requests quickly

|`* *` |manager |update my menu (add/remove condition items) | can manage the condition items I have and expand by business

|`* *` |manager |view a basic route for request | have a clearer idea on how to dispatch my deliverymen

|`* *` |manager |find requests by more files (name, condition item etc.) | have more options to locate specific requests

|`* *` |manager |trace status of requests (estimated timeline)  | can have visibility on when requests will be delivered

|`* *` |manager |have a dashboard to view succesful and failed requests  | can have insights on how my outlet is doing

|`* *` |manager |have a dynamic indication of which healthworker is available  | can send out multiple requests to many deliverymen and have improved service

|`* *` |manager |view individual customer request history  | gain insights on
what specific customers are ordering

|`* *` |manager |find my deliverman by name     | find details about the healthworker,

|`* *` |manager |see a thumbnail of healthworker  | can easily tell who is who in one glance

|`* *` |manager |edit my request  | make changes to the request

|`*` |manager |sms confirmation on succesful request  | have visibility on how my request delivery service is doing

|`*` |manager |view a map with placeholders showing requests/deliverymen routes  | have immediate view on delivery status and gain insights on my service

|`* * *` |new healthworker |see usage instructions |refer to instructions when I forget how to use the App

|`* * *` |healthworker |login/logout of application |able to protect my data and access it when I need do

|`* * *` |healthworker |list assigned requests |can tell what are the jobs I need to complete

|`* * *` |healthworker |get customer contact for requests |can contact customer if there is a need (delivery delay/incorrect address/queries etc.)

|`* *` |healthworker |view delivery route for requests |dont have to consult external service to know how to reach the destination

|`* *` |healthworker |confirm succesful request in system |the manager can track the completion of requests

|`*` |healthworker |send feedback on delivery process |the manager can relate and improve the dispatch service
...
1.  Manager requests to add request to the FoodZoom
2.  FoodZoom checks the format of the add command
3.  FoodZoom adds the request to the database
...
[discrete]
=== Use Case: Find requests by phone number

*MSS*

1.  Manager request to list requests by phone number
2.  FoodZoom shows a list of requests and their status
3.  Use case ends.

*Extensions*

* 1a. The phone number not found in the list of requests.
+
Use case ends.
...
1. Manager requests to view pending requests
2. FoodZoom lists all requests
3. Manager adds pending requests to planner
4. FoodZoom shows travel route and timing details to Manager for all requests added
...
8. Manager shown a screen showing what each healthworker is assigned
+
Use case ends.

*Extension*

* 2a.  No available requests
+
Use case ends.
...
1.  Manager requests to find the request by phone number
2.  FoodZoom shows the request
3.  Manager requests to delete the request
4.  FoodZoom deletes the request
5.  FoodZoom shows message request successfully deleted
+
Use case ends.
...
** 1a. FoodZoom shows message that request does not exist
+
Use case ends
...
6.  Delivery man checks for status of condition
7.  Deliveryman logs out to collect condition & leave for delivery
+
Use case ends.
...
* 4a. Location info is unclear to healthworker
** 4b. Delivery man searches for request to get customer's contact
...
. Should be able to hold up to 1000 requests and should respond within 2 seconds
. The system should work on both 32-bit and 64-bit environments.
. The system should only show data to the people with the <<appropriate-roles,appropriate roles>>
. The system should persist data about requests for the past year
...
=== Adding an Order
.. Prerequisites: Logged into the application. An request with name "John Lee", phone "98765432" and date "20-10-2018
00:00:00" does not already exist. To check, type `/request list` to see all existing request.

... Test case: `/request add n/John Lee p/98765432 a/311, Clementi Ave 2, #02-25, 612344 c/Physio c/Alzheimers
dt/20-10-2018 00:00:00` +
    Expected: A new request with the above details is added to the list. The command box should reflect this accordingly
    with the message "New request added: " + <details of request>. The map should show a new placeholder (or an existing
    placeholder will increase in size). Timestamp in the status bar is updated.

... Test case: `/request add n/John Lee p/98765432 a/311, Clementi Ave 2, #02-25, 612344 c/Physio c/Alzheimers
dt/21-10-2018 00:00:00` +
    Expected: A new request with the above details is added to the list. The command box should reflect this accordingly
             with the message "New request added: " + <details of request>. The map should show a new placeholder (or an
...
... Test case: `/request add n/John Lee p/98765432 a/311, Clementi Ave 2, #02-25, 612344 c/Physio c/Alzheimers
    ...
    Expected: No request is added. Error details shown in the status message (invalid date). Status bar remains the same.

... Test case: `/request add n/John Lee p/98765432 a/311, Clementi Ave 2, #02-25, 612344 dt/32-10-2018 00:00:00` +
    Expected: No request is added. Error details shown in the status message (no condition). Status bar remains the same.

... Test case: `/request add n/John Lee n/John Roe p/98765432 a/311, Clementi Ave 2, #02-25, 612344 f/Roti Prata f/Ice
Milo dt/30-10-2018 00:00:05` +
    Expected: A new request with the above details is added to the list. The name is taken as the last name given which
     is John Roe. The command box should reflect this accordingly
           with the message "New request added: " + <details of request>. The map should show a new placeholder (or an
            existing placeholder will increase in size) that gives an approximation of location. Timestamp in the
            status bar is updated.

... Test case: `/request add n/John Lee n/John Roe p/98765432 a/311, Clementi Ave 2, #02-25, 54612344 f/Roti Prata f/Ice
...
.. Prerequisites: An request with name "John Lee", phone "98765432" and date "20-10-2018
                  00:00:00" already exists
... Test case: `/request add n/John Lee p/98765432 a/311, Clementi Ave 2, #02-25, 612344 f/Roti Prata f/Ice Milo dt/20-10-2018 00:00:00` +
    Expected: No request is added. Error details shown in the status message (duplicate request). Status bar remains the
    same.


=== Selecting request
.. Prerequisites: Logged into the application. There is at least 1 request being displayed in the list.

... Test case: `/request select 1` +
    Expected: The selected request would be displayed on the display panel.

... Test case: `/request select 0` +
    Expected: No request would be selected and displayed as this is an invalid index.

=== Finding request/s
. Finding request by phone
.. Prerequisites: Having at least 1 request.
.. Test case: Valid phone without space +
    Command: `/request find p/9111` +
    Expected: Order/s with the phone `9111` and `911122113` is shown on the left panel.

.. Test case: Valid phones with space +
    Command: `/request find p/1234 9111` +
    Expected: Order/s with the phone `9111`, `911122113`, `1234`, `12345678` is shown on the left panel.

.. Test case: Empty value +
    Command: `/request find p/` +
    Expected: Error is shown in the status message. Value cannot be empty.

. Finding request by address, name, condition
.. Prerequisites: Having at least 1 request.
.. Test case: Valid address +
    Command: `/request find a/Blk 10` +
    Expected: Order/s with the address containing `Blk 10` is shown on the left panel.

.. Test case: Valid condition +
    Command: `/request find f/Chicken Rice` +
    Expected: Order/s with condition `Chicken Rice`, `Chicken Rice with Chilli` is shown on the left panel.

.. Test case: Valid name +
    Command: `/request find n/Alex Yeoh` +
    Expected: Order/s with name `Alex Yeoh`, `Alex Yeoh Jin` is shown on the left panel.

. Finding request by status
.. Prerequisites: Logged into the application. Having requests in the list with 3 different status
.. Test case: Exact Status +
    Command: `/request find st/PENDING` +
...
.. Test case: Empty status value +
    Command: `/request find st/` +
    Expected: Error is shown in the status message. Value cannot be empty.

.. Test case: Invalid status +
    Command: `/request find st/abc` +
    Expected: Error is shown in the status message. Only valid status are allowed.

. Finding request by date
.. Prerequisites: Logged into the application. Having requests added with different dates
.. Test case: Find with one datetime parameter +
    Command: `/request find dt/01-10-2018 10:00:00` +
...
.. Test case: Find with two datetime parameter +
    Command: `/request find dt/01-10-2018 10:00:00 dt/03-10-2018 14:00:00` +
...
.. Test case: Find with more than 2 datetime parameter +
    Command: `/request find dt/01-10-2018 10:00:00 dt/03-10-2018 14:00:00 dt/02-10-2018 10:00:00` +
    Expected: Order/s within and on the datetime `01-10-2018 10:00:00` and `03-10-2018 10:00:00` is shown on the left panel.

.. Test case: Empty date value +
    Command: `/request find dt/` +
    Expected: Error is shown in the status message. Value cannot be empty.

.. Test case: Only date is given. No time is given +
    Command: `/request find dt/01-10-2018` +
...
.. Test case: Invalid datetime is given +
    Command: `/request find dt/57-10-2018 10:00:00` +
    Expected: Error is shown in the status message. Date should be a valid date.

. Finding request with more than 1 argument
.. Prerequisites: Logged into the application. Having at least 1 request
.. Test case: Valid name and status value +
    Command: `/request find n/john st/pending` +
    Expected: Order/s with the name `john` and the status `PENDING` is shown on the left panel.

=== Editing an request
. Edit an request while all requests are listed
.. Prerequisites: Logged into the application. List all requests using the `/request list` command. Multiple requests in the
 list.
... Test case: Valid arguments +
    Command: `/request edit <order_index> f/Roti Canai` +
    Expected: Food of the request with the `<order_index>` will be overwritten with the condition `Roti Canai`

... Test case: Invalid request index +
    Command: `/request edit 0 n/alex` +
    Expected: Cannot edit as the request index is a non-zero unsigned integer.

... Other incorrect edit commands to try: `/request edit x f/alex` (where x is larger than the list on the left) +
    Expected: Cannot edit as the request index must be a valid index in the list shown on the left.

.. Prerequisites: Logged into the application. There is an request with the name `alex`, phone `12345678` and datetime
`01-10-2018 10:00:00`
... Test case: Edit an request to an existing request in the list +
    Command: `/request edit 1 n/alex p/12345678 dt/01-10-2018 10:00:00` +
    Expected: Cannot edit as an request with the same name, phone and datetime is already in the list of requests.

.. Prerequisites: Logged into the application. Have requests that are `Ongoing` or `Completed`
... Test case: Edit an request with status `Ongoing` or `Completed` +
    Command: `/request edit <ongoing_or_completed_order_index> f/tea` +
    Expected: Cannot edit as the request is already assign to a healthworker.

=== Deleting an request

. Deleting an request while all request are listed

.. Prerequisites: Logged into the application. List all requests using the `/request list` command. Multiple requests in the
 list. The request to be deleted is `Pending` or `Completed`.
.. Test case: `/request delete <pending_or_completed_order_index>` +
   Expected: That request is deleted from the list. Details of the deleted request shown in the status message.
   Timestamp in the status bar is updated.
.. Test case: `/request delete 0` +
   Expected: No request is deleted. Error details shown in the status message. Status bar remains the same.
.. Other incorrect delete commands to try: `/request delete`, `/request delete x` (where x is larger than the list size)
_{give more}_ +
   Expected: Similar to previous.
.. Test case: Logged into the application. Deleting an `Ongoing` request +
   Command: `/request delete o/<ongoing_order_index>`
   Expected: Unable to delete the request because it is already assigned to a healthworker.

_{ more test cases ... }_

=== Mark an request as Completed

. Marking an request as `Completed`
.. Prerequisites: Logged into the application. Already have an `Ongoing` request.
... Test case: Mark an invalid index as completed +
Command: `/request done 0` +
Expected: No request is marked as completed. Index have to a positive integer.

... Test case: Mark an valid index `Ongoing` request as completed +
Command: `/request done <order_index_of_ongoing_status>` +
Expected: Order on request list panel is marked as `Completed` and healthworker is updated (Deliveryman total assigned
requests will decrease by `1` or become `available`).

.. Prerequisites: Logged into the application. Have some `Pending` requests.
... Test case: Mark a valid index as completed but would fail. +
Command: `/request done <order_index_of_pending_status>` +
Expected: Fail to mark the request as `Completed` because only `ongoing` status can be marked as `completed`.

.. Prerequisites: Logged into the application. Have some `Completed` requests.
... Test case: Mark a valid index as completed but would fail. +
Command: `/request done <order_index_of_completed_status>` +
Expected: Fail to mark the request as `Completed` because it is already `completed`.

=== Assigning requests to a healthworker

. Assigning multiple pending requests to a healthworker

.. Prerequisite: Logged into the application. Have requests that are `Pending`, and a healthworker +
   Test case: Assigning two `Pending` requests to a healthworker +
   Command: `/assign o/<first_pending_order_index> o/<second_pending_order_index> d/1` +
   Expected: First healthworker's assigned request count increases by 2, the two requests change status to become `Ongoing`.

.. Prerequisite: Logged into the application. Have a healthworker +
   Test case: Invalid request or healthworker index +
   Command: `/assign o/0 d/1`, `/assign o/1 d/0` +
   Expected: Cannot assign because the request index is not a non-zero unsigned integer.

.. Prerequisite: Logged into the application. Have an `Ongoing` or `Completed` request +
   Test case: Assigning an `Ongoing` or `Completed` request to a healthworker +
   Command: `/assign o/<ongoing_or_completed_order_index> d/1` +
   Expected: Cannot assign because that request was already assigned to a healthworker previously.

=== Adding healthworker

.. Prerequisites: Logged into the application. Name "John" has not been used for any existing deliverymen. To check,
type `/healthworker list` to see
all existing deliverymen.

... Test case: `/healthworker add n/John` +
    Expected: A new healthworker named 'John' is added to the list. The command box should reflect this accordingly
    with the message "New healthworker added: John". Timestamp in the status bar is updated.

... Test case: `/healthworker add n/John-Paul` +
    Expected: No healthworker is added. Error details shown in the status message - non-alphanumeric characters are not allowed.
    Status bar remains the same.

.. Prerequisites: Logged into the application. A healthworker named "John" exists in the system.

... Test case: `/healthworker add n/John` +
    Expected: No healthworker is added. Error details shown in the status message.
    Status bar remains the same.

=== Deleting healthworker

.. Prerequisites: Logged into the application. There is 1 healthworker being displayed in the list. This healthworker
must not have any requests assigned to him too.
To achieve this, you can filter deliverymen by name, for example, using the default deliverymen, you can try `/healthworker find n/Chi Kao`

... Test case: `/healthworker delete 0` +
    Expected: No healthworker would be deleted as the index is invalid.

... Test case: `/healthworker delete 2` +
    Expected: No healthworker would be deleted as the index is invalid.

... Test case: `/healthworker delete 1` +
    Expected: The healthworker displayed will be deleted.

.. Prerequisites: Logged into the application. There is 1 healthworker being displayed in the list. This healthworker
has an request assigned to him.
To achieve this, you can filter healthworker by name, then assign him a `Pending` request, for example `/assign o/1 d/1`.

... Test case: `/healthworker delete 1` +
    Expected: Deliveryman would not be deleted as you are not allowed to delete healthworker with requests assigned.

=== Selecting healthworker
.. Prerequisites: Logged into the application. There is at least 1 healthworker being displayed in the list.

... Test case: `/healthworker select 1` +
    Expected: The selected healthworker would be displayed on the display panel.

... Test case: `/healthworker select 0` +
    Expected: No healthworker would be selected and displayed as this is an invalid index.
