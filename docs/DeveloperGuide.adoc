...
.  `OrderBookParser` further creates either a `OrderCommandParser` or `DeliverymanCommandParser` to parse order or healthworker command respectively. Other commands that is neither order or healthworker skip this creation of the extra Parser.
...
`/order add`, `/order edit`, `/order list`, ... .Basically all the `/order` commands and `/healthworker` commands. The
...
** Pros: Easier to implement the feature and manage the order and healthworker states.
...
|`* * *` |manager |add or remove a healthworker | can manage the deliverymen available to dispatch orders
...
|`* *` |manager |have a dynamic indication of which healthworker is available  | can send out multiple orders to many deliverymen and have improved service
...
|`* * *` |healthworker |list assigned orders |can tell what are the jobs I need to complete

|`* * *` |healthworker |get customer contact for orders |can contact customer if there is a need (delivery delay/incorrect address/queries etc.)

|`* *` |healthworker |view delivery route for orders |dont have to consult external service to know how to reach the destination

|`* *` |healthworker |confirm succesful order in system |the manager can track the completion of orders
...
... Test case: Edit an order with status `Ongoing` or `Completed` +
    ...
    Expected: Cannot edit as the order is already assign to a healthworker.
...
.. Test case: Logged into the application. Deleting an `Ongoing` order +
   ...
   Expected: Unable to delete the order because it is already assigned to a healthworker.
...
Expected: Order on order list panel is marked as `Completed` and healthworker is updated (Deliveryman total assigned
...
=== Assigning orders to a healthworker

. Assigning multiple pending orders to a healthworker

.. Prerequisite: Logged into the application. Have orders that are `Pending`, and a healthworker +
   Test case: Assigning two `Pending` orders to a healthworker +
   Command: `/assign o/<first_pending_order_index> o/<second_pending_order_index> d/1` +
   Expected: First healthworker's assigned order count increases by 2, the two orders change status to become `Ongoing`.

.. Prerequisite: Logged into the application. Have a healthworker +
   Test case: Invalid order or healthworker index +
...
.. Prerequisite: Logged into the application. Have an `Ongoing` or `Completed` order +
   Test case: Assigning an `Ongoing` or `Completed` order to a healthworker +
   Command: `/assign o/<ongoing_or_completed_order_index> d/1` +
   Expected: Cannot assign because that order was already assigned to a healthworker previously.
...
To achieve this, you can filter healthworker by name, then assign him a `Pending` order, for example `/assign o/1 d/1`.

... Test case: `/healthworker delete 1` +
    Expected: Deliveryman would not be deleted as you are not allowed to delete healthworker with orders assigned.
